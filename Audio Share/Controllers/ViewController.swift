...
/*class ViewController: UIViewController, SPTSessionManagerDelegate {
    // spotifyClientId loaded from Secrets.xcconfig → Info.plist (see Constants.swift)
    let SpotifyRedirectURL = URL(string: "yourapp://callback")!

    lazy var configuration = SPTConfiguration(clientID: spotifyClientId, redirectURL: SpotifyRedirectURL)
    lazy var sessionManager: SPTSessionManager = {
        if let tokenSwapURL = URL(string: "your-token-swap-url"),
           let tokenRefreshURL = URL(string: "your-token-refresh-url") {
    ...
    func sessionManager(manager: SPTSessionManager, didInitiate session: SPTSession) {
        ...
        // Use session.accessToken to make API requests
    }
